...
To my parents and my brother, that have always helped me push through hard moments.
...
To all of my friends and colleagues that have accompanied me these years.
...
The background regarding the machine learning models employed, the considered technical indicators, and the error metrics that will be used to evaluate the performance, will be discussed in \textit{\hyperref[chap:02]{Chapter 2}}. The research project will test \textit{Linear Regression} and \textit{Lasso Regression} for linear models, and the \textit{Random Forest Regressor} for a non-linear comparison. The feature engineering phase will consist of enhancing the dataset with a variety of trend, momentum, and volatility indicators. Finally, the evaluation of the different models performance will be done with the mean absolute error, the mean squared error, the coefficient of determination ($R^2$) and an expectation shortfall-like metric.
...
The core of this project revolves around an experimental system design presented in \textit{\hyperref[chap:03]{Chapter 3}}. The design integrates the use of a sliding window training approach for traditional machine learning models, with the use of expressive features created from financial indicators. This experimental use of technical analysis in the feature engineering phase is intended to investigate the effectiveness of these indicators applied in a different field. The idea is to aid the model training process by exposing it to different indicators that describe the trend, momentum and volatility of the time series. The model performance results will be discussed in depth in \textit{\hyperref[chap:04]{Chapter 4}}.

% - Cap 5 regulatory review
% - Cap 6 socio-economic impact
Finally, in \textit{\hyperref[chap:05]{Chapter 5}}, the availability and usage of the data is reviewed, as well as the software tools and licensing requirements involved in the development of the project. The socio-economic impact of the project is addressed in \textit{\hyperref[chap:06]{Chapter 6}}, which also presents a Gantt diagram that will describe the project plan, along with its estimated budget. The document culminates with the \textit{\hyperref[chap:07]{Chapter 7}} conclusion, where the final remarks over the obtained results are made, and potential avenues for future work is discussed.
...
% Energy related predictions have been a matter of study for quite some time.

% There are a variety of strategies to predict energy prices. There are various alternatives to machine learning, such as deep learning neural networks, or statistical time-series models. These are radically different approaches to the one taken in this project.

% - ml prediction of wind power generation
% \cite{ml-tfg-uc3m-wind}

% - Deep Learning for price predictions
% \cite{malasyia-neural-nets}

% - LSTMs - Temporal Series
% Nogales et al. \cite{time-series-energy} proposed dynamic regression and transfer function models for next-day electricity price forecasting, applying their methodology to the Spanish and Californian markets.

% - Energy price construction and predictions
% \cite{electricity-price-uc3m-tfg}
...
In the subject of price forecasting, many studies such as the one from Nogales et al. \cite{time-series-energy}, have focused on statistical time-series models. The research article presented dynamic regression and transfer function models that employed historical price patterns to predict next-day electricity prices. They applied their methodology to the Spanish and Californian electricity markets in an attempt to capture short-term fluctuations.

Other research has focused on forecasting the production of renewable wind energy, to enable its integration into the electrical system. A uc3m alumni, Ouanani Allachi developed a machine learning-based model to predict wind power generation \cite{ml-tfg-uc3m-wind}, which is particularly important as renewable account for a growing share of electricity generation.

Other techniques such as deep learning have been increasingly applied to energy forecasting problems. For instance, Shapia et al. \cite{malasyia-neural-nets} applied deep neural networks to predict energy consumption in smart buildings, demonstrating the ability of these models to capture complex nonlinear relationships in energy data.

In regards to the Spanish electricity market, another uc3m alumni, Marín Lancha, analyzed the price formation mechanisms and behavior of electricity prices \cite{electricity-price-uc3m-tfg}. This project provided useful insights into the factors that are responsible for the price variations of energy in Spain.
...
% The approach relies on machine learning models combined with technical indicators extracted from historical price data, with the goal of finding a balance between model complexity and predictive accuracy.
...
The \textit{lasso regression} experiments expanded upon the different temporal window configuration training batches, by also testing across multiple values of its regularization hyperparameter \textit{alpha}. In this model, the objective was to assess its impact on feature selection and model generalization.
...
This section presents the best results obtained across the different models. It provides a general overview of the performance potential of each of the top model configurations, analyzing the impact of both feature engineering and hyperparameter tuning on the final outcomes. All of the following results presented correspond to the 99\textsuperscript{th} percentile of prediction errors, meaning the top 1\% of the largest errors were excluded. This was chosen in order to fairly represent performance while minimizing the influence of extreme outlier predictions that are caused by outliers or isolated model errors

% no tienen por qué ser errores del modelo, cuando hay outliers lo que ocurre es que limitas la capacidad expresiva de tu modelo para que funcione bien la mayor parte del tiempo. Hay una frase de un portero de fútbol mítico de lo '80 que decía que cuando un portero para muchos tiros a la escuadra (algo espectacular) es un mal portero porque lo normal si paras muchos tiros a la escuadra es que estés mal colocado la mayor parte del tiempo,

The considerations taken to decide the top performing model were simple and focused on practical predictive performance. A custom score was created that averages out the ranking of the 4 calculated metrics, the Minimum Average Error (MAE), Mean Squared Error (MSE), the coefficient of determination ($R^2$), and our metric similar to Expectation Shortfall (ES-like), which in our experiments will account for average of the largest 5\% of errors inside the considered 99\textsuperscript{th} percentile. The MAE, MSE and ES-like metric are ranked by ascending order, meaning the lower the metric, the higher the rank. In contrast, $R^2$ is ranked by descending order, as values closer to 1 correspond to smaller prediction errors.


For consistency and ease of comparison with the other metrics, all MSE values are presented in their root form (RMSE), which retains the same units as the original data (€/MWh). An example of the error and accuracy metric calculation process can be found in \textit{\hyperref[app:appendix_i_example_metric_calculation_process]{Appendix I}}). 


\noindent \textbf{Linear Regression}

Starting with the testing, the first model used will be linear regression. For this model, it is not necessary to tune any hyperparameters, since it does not have any. The training batches were limited to testing different sliding windows.
...
% The plot for the best baseline performer is the following:
...
A visual inspection indicates that the model correctly tracks the time series, performs reasonably well during periods of relatively stable prices (2018-2020 and parts of 2023-2024), where predicted values closely follow actual prices. We can observe that the model underperforms during the major price spikes in 2022, suggesting that it struggles with volatility. At times, there also appears so be some lag in predictions, especially during rapid price changes, where the model appears to react to trends rather than anticipate them. This is potentially due to the 30-day long window, and short 1-day lag price, that might not give enough context. As a baseline model, this simple approach provides a good starting point for comparison.


% with room for improvement that will be tackled with a feature engineering process. These metrics will provide the improved models with

% The poor performance during volatile periods suggests the need for additional features or different modeling approaches that can better capture market dynamics.

% This baseline establishes that more advanced models (ensemble methods, neural networks, or time series models) would likely be necessary for better performance during volatile market conditions.


The next experiment consists of using the more complex feature matrix, which includes the calculated technical indicators. This will review the potential improvements and variations of the feature engineering training run, in comparison to the baseline run, while recognizing the trade-off between capturing additional underlying patterns and introducing noise or redundancy that can lead to overfitting in linear regression. The temporal windows that will be tested are: sliding\_windows = [7, 10, 15, 30, 90] and lag\_price\_windows = [1, 2, 4, 6]. The best feature engineering results, and the plot of its best performer are the following:
...
% The plot for the best feature engineering performer is the following:
\begin{figure}[H]
    \centering
...
From the results we can observe a clear improvement when dealing with price surges during volatile market conditions. The predictions appear to respond more quickly to price changes, with less lag compared to the baseline model. This behavior can be noticed in the 2021 price rise, where the baseline model tended to underestimate values, while the feature engineered one captured well the day to day variations, including local peaks. In general there are consistently fewer over or under-estimations across the whole time series.
...
The second model considered in this evaluation, Lasso regression is also a linear model that penalizes unused features. It does have a tunable hyperparameter \textit{alpha}, enabling manual model optimization. A selection of parameter values will be tested in every sliding window iteration, noting down the \textit{alpha} value that renders the result with the minimum error to the actual data point. In addition to all the previous temporal window configurations, the following \textit{alpha} values were tested: alphas = [0.01, 0.1, 1, 10, 100, 1000]. The best baseline model configurations, and the top performer plot are the following:
...
From the results from this training we can observe that the predictions are very similar to the linear regression baseline. This is due to the best performant Lasso model being attained with the lowest \textit{alpha} hyperparameter (0.01), meaning it is not performing any feature selection, effectively working as the previous linear regression model. The other two top models do employ some regularization, displaying the preference of the 30-day sliding window, by 1-day lag window of the Lasso model. 
...
This demonstrates that well-engineered features can make regularization almost irrelevant. With the right temporal features, even simple linear models can achieve good performance in electricity price forecasting.

% Robust model: The insensitivity to alpha values (even very high regularization at 1000) suggests the model has identified truly relevant features without overfitting.

% Strong baseline: With R² = 0.819, this represents excellent performance for a linear model, making it a strong benchmark for comparing more complex algorithms.
...
The last model that was experimented with was the Random Forest regressor. The two main parameters that we will focus on tuning are the number of trees in the forest (\verb|n_estimators|) and the maximum depth of the tree branches (\verb|max_depth|). Similar to the training process of Lasso regression, for every sliding window iteration each combination of parameters will be tested. Likewise, we will save the combination of parameters with the best result, with the least error to the actual data point. All of the previous temporal windows will be considered, alongside n\_estimators = [100, 200, 300, 400] and max\_depths = [None, 10, 20, 30]. The results for the best baseline runs, and the plot of the best performer are the following:
...
Concluding the main results overview, the following table presents the best performing run of each model based on the best \textit{RMSE}, which is considered to be the most representative metric to use when comparing different models over the same dataset. In the case of the tie observed in linear regression, the \textit{coefficient of determination} metric was used, since a higher $R^2$ indicates a better "predicted to actual" value fit.
\begin{table}[H]
...
% - Did the FE work? Not really, the FE models slightly edge the baseline models in the average error, with the other metrics consistenltly worsening in accuracy 
...
% - Is there any common SW or LW parameters? For Linear and Lasso same temporal window parameters because lasso 0.01 is basically the same as linear
...
Note: The code for all of these experiments can be found on my GitHub repository \cite{rodrigodelamaTFG}.
...
% In this section the complete results are analyzed. The general distribution of the results will be reviewed in order to gather insights into the results from each model. The effect of tuning each parameter will be reviewed to further understand each of the results.

% The effect of each parameter in the different training batches will be discussed, how it affected the error rates, and possibly why this might have happened.

% The results will also be considered across a range of lower percentiles to determine our overall success rate across the different temporal configurations. The percentiles that will be considered are the 100\textsuperscript{th}, 99\textsuperscript{th}, 95\textsuperscript{th}, 85\textsuperscript{th} and 75\textsuperscript{th}.

In this section we present a comprehensive analysis of the data processing pipeline performance across all experiments. We compare all configurations, gathering insights about the impact of the different temporal window setups, the feature engineering process, and hyperparameter tuning. The evaluation primarily includes our calculated metric, MAE, MSE, $R^2$, and our Expectation Shortfall-like metric to capture the models’ performance across all possible combinations. Once again, the following results are for just the 99\textsuperscript{th} percentile in order to best represent the actual models performance, ignoring isolated model errors.
% An overview of the models' performance on a percentile-based analyses will also be mentioned, to understand the overall success rate of the best temporal configurations
...
Being linear regression our first model, no hyperparameters were considered, simplifying the distribution representation of the performance metrics. As we can only tune two parameters, \verb|sliding_window| and \verb|lag_price_window|, the configurations can be plotted over a 2D heat map. The figure representing the the baseline model distribution is the following:
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{imagenes/regression_baseline_metrics_spread_analysis.png}
    \label{fig:regression_baseline_metrics_spread_analysis}
    \caption{Baseline Linear Regression Metrics Distribution}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{imagenes/regression_fe_metrics_spread_analysis.png}
    \label{fig:regression_fe_metrics_spread_analysis}
    \caption{Feature Engineering Linear Regression Metrics Distribution}
\end{figure}
...
\noindent \textbf{Lasso Regression}
% Talk about the temporal window configs and the effect of FE, consider the different alphas across iterations

To be able to understand the lasso result distribution, we must consider and tune its hyperparameter \textit{alpha}. The best score results for each temporal window combination are presented, displaying below the \textit{alpha} parameter with which they were obtained with. An additional heat map will be included to exclusively display the most common \textit{alpha} values across the different temporal window configurations. This will attempt to provide the extra dimensionality required to understand the Lasso regression results. Starting with the baseline results, the following figure displays the best results for each window, and the respective \textit{alpha} with which they were obtained:
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{imagenes/lasso_baseline_best_alpha_heatmaps.png}
    \label{lasso_baseline_best_alpha_heatmaps}
    \caption{Baseline Lasso Best Alpha Metrics Distribution}
\end{figure}

For the the analysis of the Lasso model, the hyperparameter value selection is probably the most relevant result. In the case of these baseline results, we can observe that the best \textit{alpha} is the lowest value considered in the training batches. An \textit{alpha} of 0.01 barely introduces any regularization, meaning instead of reducing the effect of certain features to zero, it considers them all as relevant, effectively working as Linear regression. The results are in line with the temporal window dimensionality, since the narrower the lag day window is, the less it can regularize. This is due to the fact that in the baseline runs, the feature matrix is as wide as the lag price window value, as it does not have any other features. These results make sense since the Lasso model cannot regularize properly when it has fewer features. Irrespective of this, we can observe that the best performing temporal window configuration is the exact same as with Linear regression, indicating that the Lasso model was unable to properly utilize lag day prices as features to aid in the model training.

The following figure presents the results of the feature engineering training batches:
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{imagenes/lasso_fe_best_alpha_heatmaps.png}
    \label{fig:lasso_fe_best_alpha_heatmaps}
    \caption{Feature Engineering Lasso Best Alpha Metrics Distribution}
\end{figure}

In this last linear model analysis can observe that the best performance was obtained in the 7-day sliding window, with 6 lag day prices. In this occasion, as discussed in the previous section, the top performers used the same temporal window, but with very different \textit{alpha} values. This best result can be observed with the lowest MAE, MSE and highest, most accurate, $R^2$ score, with the exception of the ES-like metric, which as towards the higher end of the values. As mentioned earlier, this indicates that the feature engineering selection was optimal, as the results were independent of the \textit{alpha} value. Concluding with the linear models analysis, the regularization of the Lasso hyperparameter was not crucial in the performance of the model.

\noindent \textbf{Random Forest}
% Talk about the temporal window configs and the effect of FE, include n\_estimators and max\_depth

Our final analysis will review the Random Forest regressor. In this non-linear model we will have with one more parameter than with Lasso regression, adding more complexity to the results. The first parameter will analyze the effect of tuning the number of estimators, \verb|n_estimators|, arguably the most important parameter, which determines the number of "trees" that will be built in the forest. We will also consider the effect of limiting the maximum depth, \verb|max_depth|, of each decision tree. Tuning this helps to prevent individual trees from becoming too complex and overfitting to the training data, which contributes to the overall model's generalization ability.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{imagenes/random_forest_baseline_best_config_heatmaps.png}
    \label{random_forest_baseline_best_config_heatmaps}
    \caption{Baseline Random Forest Metrics Distribution}
\end{figure}

% Recalling the previous sections results, 
In these results we can observe that the best result that was obtained was for the 90-day sliding window, with the 6-day lag price window. The non-linear nature of the model clearly benefited the most out of more features in this baseline run with the simple matrix. Once again, the top results are complex to display, as they employed the same temporal window, with different \verb|n_estimators| and \verb|max_depth| parameters.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{imagenes/random_forest_baseline_hyperparameter_analysis.png}
    \label{random_forest_baseline_hyperparameter_analysis}
    \caption{Baseline Random Forest Hyperparameter Analysis}
\end{figure}

From these distributions we can observe that lower tree depths were typically preferred among the top performers, which were not as sensitive to the number of estimators shown in the mixed \verb|n_estimators| results. The best performing models clearly employed parameter combinations from the top results, with 300 estimators actually being more prevalent among them, rather than the 400 estimators employed by the two best models. These are interesting findings that proved that for this sliding window approach, a higher number of estimators, with shallower tree depths were optimal for the best predictors on average.

For the final training batch, the feature engineering results are presented in the following figure:
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{imagenes/random_forest_fe_best_config_heatmaps.png}
    \label{fig:random_forest_fe_best_config_heatmaps}
    \caption{Feature Engineering Random Forest Metrics Distribution}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{imagenes/random_forest_fe_hyperparameter_analysis.png}
    \label{random_forest_fe_hyperparameter_analysis}
    \caption{Feature Engineering Random Forest Hyperparameter Analysis}
\end{figure}
...
\noindent \textbf{The Effect of Feature Engineering}
...
The code that created the all of the graphs for this analysis can be found here on my GitHub repository \cite{rodrigodelamaTFG}.
...
Another example sector that consumes vast amounts of electricity is the telecommunications industry, along with its data centers and IT services. In today's modern society, we cannot afford to ever shut down crucial communications infrastructure, or data center servers, which aim for the highest possible availability.
The electric mobility and logistics sector is another key example of an industry that is not tolerant to shutdowns and failures, since they depend on easy and relatively affordable access to electricity. For these various sensitive industries, forecasting models capable of anticipating these fluctuations can provide a buffer for strategic planning, risk management, and preventive measures.
...
\begin{figure}[H]
    ...
    \begin{ganttchart}[
        ...
        \ganttbar{Initial Idea Discussions}{2024-09}{2024-10} \\
        \ganttbar{System Design}{2024-10}{2024-12} \\
        \ganttbar{Coding}{2024-12}{2025-02} \\
    
        % \ganttgroup{Project Phase 2}{2025-01}{2025-06} \\
        \ganttbar{Code Review \& Corrections}{2025-02}{2025-05} \\
        \ganttbar{Results Gathering}{2025-03}{2025-05} \\
        \ganttbar{Thesis Redaction}{2025-05}{2025-06}
...
\section{Revisiting the Objectives of the Research} % Or Project
...
% It did work - kind of:
% Slight improvements in the Linear models, but with the trade-off of worse ES-like values. These were reasonably fast and easy to compute, easy to test multiple configurations.
% In Random Forest better across al metrics, but not amazing. These were extremely taxing in terms of the compute required to process the model. Not really worth it across the board, only sparingly better.


The main objective of this project was to explore the feasibility of predicting electricity prices using Machine Learning models, leveraging feature engineering through technical analysis indicators. The intention was to assess whether they could be effectively used to predcit time series values, and if so, if such an approach could provide meaningful predictive power while maintaining reasonable computational efficiency.
...
Another potential avenue is the development of a hybrid model architecture that combines the strengths of different algorithms. For example, combining the reliable but cautious Random Forest predictions, with the sensitivity of linear models. This model could potentially be more consistent, while still being able to capture sharp price variations, creating a more balanced and accurate predictor.
...
\chapter* {Appendix I: Example of the Metric Calculation Process}
\label{app:appendix_i_example_metric_calculation_process}
