...
% Finally, in chapter 5, we will discuss the availability of the data and its usage. In addition, all of the software and licensing relevant to the development of this project will be outlined.

% The socio-economic impact of the project will be discussed in chapter 6, along with a project plan and its associated budget.

% Culminating with the conclusion in chapter 7 that will review the results, and discuss potential future work.

Finally, in \textit{\hyperref[chap:05]{Chapter 5}}, we will review the availability and usage of the data, as well as detail the software tools and licensing requirements involved in the development of the project. The socio-economic impact of the project will be addressed in \textit{\hyperref[chap:06]{Chapter 6}}, also presenting a Gantt diagram that will describe the project plan, along with its estimated budget. The document will culminate with the \textit{\hyperref[chap:07]{Chapter 7}} conclusion, where the obtained results will be analyzed, and potential avenues for future work will be discussed.
...
% Nearly identical results: The predictions are virtually indistinguishable from the Linear Regression baseline, suggesting that with the current feature set and alpha value (0.01), Lasso is not performing significant feature selection.

% Minimal regularization effect: The low alpha value (0.01) means the L1 penalty is quite weak, so Lasso behaves almost like ordinary linear regression in this case.
...
The code that created the graphs for this analysis can be found here on my GitHub repository \cite{rodrigodelamaTFG}.
